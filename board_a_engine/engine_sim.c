...
#define ENG_ID_RPM        0x280
#define ENG_ID_SPEED      0x1A0
#define ENG_ID_COOLANT    0x288
#define ENG_ID_KEEPALIVE  0x300   // 임시. 계기판 필수 ID 확정되면 변경
...
static void EngineSim_SendRpm(void)
{
    ...
    if (CAN_BSP_Send(ENG_ID_RPM, data, 2) == HAL_OK)
        engine.tx_count++;
}
...
static void EngineSim_SendSpeed(void)
{
    ...
    CAN_PutU16LE(data, 0, engine.speed);

    if (CAN_BSP_Send(ENG_ID_SPEED, data, 2) == HAL_OK)
        engine.tx_count++;
}
...
static void EngineSim_SendCoolant(void)
{
    ...
    data[0] = engine.coolant;

    if (CAN_BSP_Send(ENG_ID_COOLANT, data, 1) == HAL_OK)
        engine.tx_count++;
}
...
static void EngineSim_SendKeepAlive(void)
{
    ...
    if (CAN_BSP_Send(ENG_ID_KEEPALIVE, data, 8) == HAL_OK)
        engine.tx_count++;
}
