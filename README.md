...
STM32F429ZI 기반 5개 ECU 보드와 Volkswagen Golf 6 계기판을 CAN으로 연결하는 교육용 프로젝트입니다.

목표는 보드 A가 엔진 데이터를 만들고, 보드 D가 BCM/Body 데이터를 만들며, 보드 E가 Safety/ADAS 위험 상태를 판단하고, 보드 B가 Central Gateway로 CAN1과 CAN2를 중계하며, 보드 C가 UDS 진단 요청을 주고받는 구조입니다.
...
```text
[보드A - 엔진 ECU 시뮬 + 페달 입력]   [보드D - BCM / Body Control Module]   [보드E - Safety / ADAS]
  가변저항(액셀/브레이크)              Body CAN 신호 생성 및 주기 송신
  -> RPM, Speed, Coolant 주기 송신     Golf 6 0x390 방향지시등 L/R, 도어, 하이빔, 안개등   전방/후방 거리, 차선, 급제동, 센서고장 판단
  사용자가 돌리면 RPM 실시간 변화     DIP 스위치 4개 -> FL/FR/RL/RR 도어 상태
  IGN ON 신호 송신                    모멘터리 버튼 -> 클러스터 화살표 점멸                  0x3A0 ADAS_Status 송신
...
[보드B - Central Gateway]
  ...
  이상 감지: RPM 초과 및 ADAS 위험 시 경고 메시지 CAN2 송신
  버스 통계 UART 출력 (트래픽 로거)
  보드A/보드D/보드E 메시지 수신 및 필요 시 전달
          |
==========|================ CAN2 (Diagnostic/Cluster, 500kbps) =============
          |                                                  |
[보드C - UDS 진단 서버]                      [Volkswagen Golf 6 계기판]
  SID 0x22 응답                               RPM 바늘 추종
  VIN / RPM / Speed / ADAS DID                속도 바늘 추종
  UART CLI <- PC                              냉각수 온도
  NRC 처리, clear dtc                         경고등 ON/OFF
                                            방향지시등 / 도어 / 하이빔 / 안개등 표시
```
...
flowchart LR
    ...
    E["보드E<br/>Safety / ADAS ECU<br/>전방/후방 거리, 차선, 급제동<br/>0x3A0 ADAS_Status"]
    B["보드B<br/>Central Gateway<br/>CAN1 수신<br/>라우팅/이상감지/로깅"]
    C["보드C<br/>UDS Diagnostic<br/>UART CLI<br/>SID 0x22, clear dtc"]
...
sequenceDiagram
    ...
    A->>E: Speed/IGN reference<br/>0x100
    D->>B: Body status<br/>Golf 6 0x390 mGate_Komf_1
    E->>B: ADAS_Status<br/>0x3A0 risk/fault/distance
    B->>K: Gateway routed cluster messages
    B->>K: Warning event<br/>0x480 mMotor_5 when ADAS risk >= 2
    C->>B: UDS ReadDataByIdentifier request
    B->>C: UDS positive/negative response
    C->>B: UDS ClearDiagnosticInformation
```

**메시지를 주고받는 기준**

- CAN1은 보드 A, 보드 B, 보드 D가 같이 물리는 Powertrain/Body 버스입니다.
- 보드 E도 CAN1에 물려 `0x3A0 ADAS_Status`를 보드 B로 송신합니다.
- CAN2는 보드 B, 보드 C, Golf 6 계기판이 물리는 Diagnostic/Cluster 버스입니다.
- 보드 A, 보드 D, 보드 E는 직접 CAN2로 가지 않고, 보드 B가 필요한 메시지만 CAN2로 넘겨야 합니다.
...
- 계기판을 실제로 움직이려면 보드 B가 Golf 6 DBC 메시지를 CAN2로 보내야 합니다. K-Matrix/DBC 기준 `0x480`은 `mMotor_5`이므로 ADAS 경고는 해당 프레임의 warning 성격 bit로 변환합니다.
...
| **Safety/ADAS ECU (보드 E)** | TBD | 위험 상황 판단 | 전방/후방 거리, 차선 이탈, 급제동, 센서 고장 판단, `0x3A0 ADAS_Status` 송신 |
...
| 보드E | STM32 F429ZI | Safety / ADAS ECU | CAN1 |
...
| 보드 C UDS | UART CLI, SID `0x22` ReadDataByIdentifier 요청/응답, ADAS DID 조회, `clear dtc` 요청 |
| 보드 D BCM | GPIO 입력, IGN 수신, IGN ON 이후 Body 송신, Golf 6 `0x390 mGate_Komf_1` 패킹 구현 |
| 보드 E Safety/ADAS | ADC/GPIO 입력 기반 전방/후방 거리, 차선 이탈, 급제동, 센서 고장 판단, `0x3A0 ADAS_Status` 송신 |
...
| `0x3A0` | 8 | 보드 E | 보드 B | 100ms | 프로젝트 내부 `ADAS_Status`: risk/fault/front/rear |
| `0x480` | 8 | 보드 B | 계기판 | ADAS 상태 반영 | Golf 6 `mMotor_5`: warning lamp/text bit overlay |
...
현재 Board C CLI와 Gateway ADAS UDS 응답은 `0x714/0x77E`를 사용합니다. Gateway는 `0x7E0/0x7E8` 요청도 보조로 받을 수 있게 되어 있습니다.
...
| `0x480` | `mMotor_5` | 보드 B | `MO5_HLeuchte`, `MO5_Heissl`, `MO5_TDE_Lampe`, `MO5_Motortext3` | ADAS warning/fault overlay |
...
`0x3A0`은 Golf6 K-Matrix에서 `mBremse_10`으로도 쓰이므로, 이 프로젝트에서는 CAN1 내부 Safety 프레임으로 사용합니다. Gateway는 기본적으로 원본 `0x3A0`을 CAN2로 포워딩하지 않고 `0x480 mMotor_5` warning bit로 변환합니다.
...
4. 보드 E를 연결하고 CAN1에서 `0x3A0`이 100ms마다 나오는지 확인합니다.
5. 보드 B를 연결하고 CAN1 RX count가 증가하는지 UART 로그로 확인합니다.
6. 보드 B 라우팅 구현 후 CAN2에서 `0x280`, `0x1A0`, `0x288`, `0x390`이 보이는지 확인합니다.
7. 보드 E에서 speed >= 40km/h, front <= 30cm 조건을 만들고 CAN2 `0x480` warning frame이 나오는지 확인합니다.
8. 보드 C에서 `read adas`, `read fault`, `clear dtc`를 실행해 Gateway ADAS 진단 응답을 확인합니다.
