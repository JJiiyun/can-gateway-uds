...
│   └── board_e_safety/         # 보드 E 단위 빌드 프로젝트
├── docs/
├── tools/
...
firmware/board_e_safety/build/Debug/board_e_safety.elf
```

## 5. 통합 빌드

통합 빌드는 루트 `CMakeLists.txt`에서 다섯 보드를 모두 configure/build합니다. `common/`을 수정했거나 여러 보드 간 인터페이스를 건드렸다면 반드시 통합 빌드를 돌립니다.
...
cmake --build build/Debug --target build_board_e_safety --parallel
...
Remove-Item -Recurse -Force firmware/board_e_safety/build -ErrorAction SilentlyContinue
...
| `unit-build` | 보드 A/B/C/D/E를 matrix로 각각 단위 빌드 |
...
| 상황 | 통과해야 하는 항목 |
|---|---|
| 보드 A/B/C/D/E 중 하나만 수정 | 해당 보드 단위 빌드 + 통합 빌드 |
| `common/` 수정 | 다섯 보드 단위 빌드 + 통합 빌드 |
| 문서만 수정 | `layout` 통과 |
...
| `[engine-sim]` | `firmware/board_a_engine/MyApp` |
| `[gateway]` | `firmware/board_b_gateway/app` |
| `[uds]` | `firmware/board_c_uds/board_c_uds` |
| `[body]` | `firmware/board_d_body/src` |
| `[safety]` | `firmware/board_e_safety/src` |
| `[cluster]` | 계기판 관련 코드 |
| `[docs]` | 문서 |
