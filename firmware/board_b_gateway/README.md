...
| Safety bridge | Board E `0x3A0` ADAS 상태를 진단 상태로 저장하고 필요 시 `0x480` warning 및 `0x5D6` 파킹 어시스트 경고음 송신 |
...
| `0x480` | Gateway | ADAS 경고 (위험도/센서 오류 경고등) |
| `0x5D6` | Gateway | ADAS 경고음 (위험도 2 이상 시 파킹 어시스트) |
...
따라서 Board B의 역할은 CAN2에서 오가는UDS 요청/응답을 방해하지 않는 것입니다. `gateway_uds_server.c/h`와 Board B UDS 응답 ID/DID 정의는 제거되었습니다.

## Safety Bridge - ADAS 경고 처리

Board E에서 ADAS 상태(위험도, 센서 오류, 충돌 감지 등)를 `0x3A0`으로 전송하면, Gateway의 Safety Bridge는:

1. **경고등 송신** (`0x480` - Motor_5):
   - 위험도 2 이상, 센서 오류, 또는 활성 오류 발생 시 경고등 비트 설정
   - 위험도 3이면 추가로 고위험 비트 설정
   - 100ms 주기로 송신

2. **경고음 송신** (`0x5D6` - 파킹 어시스트):
   - 위험도 2 이상 시에만 경고음 신호 생성
   - 위험도 2: 600ms 주기로 음성 펄스
   - 위험도 3: 300ms 주기로 음성 펄스 (더 자주 울림)
   - 각 펄스는 100ms 지속

3. **DTC 누적**:
   - 센서 오류나 활성 오류 발생 시마다 DTC 비트맵에 누적
   - UDS `0x14` (Clear DTC) 명령으로 리셋 가능
