...
`defaultTask`는 `uartInit()`, `cliInit()`, `CliCmd_Init()`, `EngineSim_Init()`을 수행합니다. `EngSimTask`는 `EngineSim_Task()`를 실행합니다.
...
현재 계기판 반응 확인을 위해 coolant CAN 값은 계산 없이 고정 정수로 송신합니다.

```c
#define ENGINE_COOLANT_CAN_FIXED_VALUE  90U
```

따라서 `0x288` payload의 `byte[1]`은 항상 `90`, 즉 `0x5A`입니다.
...
RPM warning 판단이 필요하면 `0x280`의 `byte[2..3]`를 little-endian으로 읽고 `rpm = raw / 4`로 해석합니다. Speed 로그가 필요하면 `0x1A0`의 `byte[2..3]`를 읽고 `speed = raw / 160`으로 해석합니다.
